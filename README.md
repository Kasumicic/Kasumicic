### 👋 Привет, я Вячеслав!

Системный администратор и Linux-энтузиаст из Красноярска. Обожаю копаться в том, как работают сложные вещи "под капотом", и автоматизировать всё, до чего дотянутся руки. Верю, что лучший скрипт — тот, который работает, пока ты спишь.

---

### 🚀 Мои навыки и инструменты

*   **Операционные системы:** Linux (Ubuntu, CentOS), Windows Server
*   **Скриптинг и автоматизация:** Bash, Python
*   **Сети:** TCP/IP, DNS, DHCP
*   **Анализ данных:** Pandas, Scikit-learn, LightGBM, Jupyter

![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnubash&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

---

### 📫 Как со мной связаться

*   **Email:** `slavkaplay2@gmail.com`
*   **Telegram:** `@pom0gite_mne`

---

<div align="center">
  <img src="https://raw.githubusercontent.com/Kasumicic/Kasumicic/main/.github/assets/ezgif-15e68e127ee02e.gif" alt="ЭТО ЧТО, ЛИНУС ТОРВАЛЬДС?" width="600"/>
</div>
